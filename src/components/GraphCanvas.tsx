...
export default function GraphCanvas({
  ...
  return (
    <div
      ref={wrapRef}
      className="relative overflow-hidden rounded-2xl border border-paper-300"
      style={{
        background:
          "radial-gradient(ellipse at 30% 20%, #fdf9f0 0%, #f8f1e2 55%, #f2e8d4 100%)",
      }}
    >
      {/* 줌 컨트롤 */}
      <div className="absolute right-3 top-3 z-10 flex flex-col gap-1.5">
        <button
          className="btn-ghost h-9 w-9 bg-white/90 p-0"
        ...
        <button className="btn-ghost h-9 w-9 bg-white/90 p-0" onClick={resetView} title="배치 다시 만들기">
          ⟳
        </button>
      </div>

      {/* 범례 */}
      <div className="pointer-events-none absolute bottom-3 left-3 z-10 flex flex-wrap items-center gap-x-3 gap-y-1 rounded-xl bg-white/80 px-3 py-1.5 backdrop-blur">
        {(Object.keys(BLOCK_TYPE_META) as BlockType[]).map((t) => (
          <span key={t} className="flex items-center gap-1 text-xs font-medium text-stone-600">
            <span
              ...
              style={{ background: BLOCK_TYPE_META[t].node }}
            />
      ...
      {tooltip && !dragRef.current && (
        <div
          className="pointer-events-none absolute z-20 w-56 rounded-xl border border-paper-300 bg-white/95 p-3 shadow-card backdrop-blur"
          style={{
        ...
        >
          <div className="flex items-center gap-1.5 text-sm font-bold text-stone-800">
            <span>{BLOCK_TYPE_META[tooltip.block.type].icon}</span>
            {tooltip.block.name}
            <span
              className="ml-auto rounded-full px-1.5 py-0.5 text-[10px] font-semibold text-white"
              style={{ background: BLOCK_TYPE_META[tooltip.block.type].node }}
            >
          ...
          <p className="mt-1 text-[10px] text-stone-400">클릭하면 자세히 볼 수 있어요</p>
        </div>
      )}
      ...
      >
        <defs>
          <pattern id="dots" width="30" height="30" patternUnits="userSpaceOnUse">
            <circle cx="1.5" cy="1.5" r="1.2" fill="rgba(140,115,80,0.13)" />
          </pattern>
          ...
          <filter id="softShadow" x="-40%" y="-40%" width="180%" height="180%">
            <feDropShadow dx="0" dy="1.5" stdDeviation="2" floodColor="#7a6240" floodOpacity="0.30" />
          </filter>
          <filter id="hullBlur" x="-30%" y="-30%" width="160%" height="160%">
            <feGaussianBlur stdDeviation="13" />
          </filter>
          {(Object.keys(BLOCK_TYPE_META) as BlockType[]).map((t) => (
            <radialGradient key={t} id={`grad-${t}`} cx="35%" cy="30%" r="80%">
              <stop offset="0%" stopColor="#ffffff" stopOpacity="0.85" />
              <stop offset="45%" stopColor={BLOCK_TYPE_META[t].node} stopOpacity="0.55" />
              <stop offset="100%" stopColor={BLOCK_TYPE_META[t].node} />
            </radialGradient>
          ))}
        </defs>
        ...
        <g transform={`translate(${pan.x},${pan.y}) scale(${scale})`}>
          ...
          {orgHulls.map((h) => {
            ...
            return (
              <g key={h.org.id} opacity={isDim ? 0.15 : 1} style={{ transition: "opacity .2s" }}>
                <path
                  d={h.path}
                  fill={BLOCK_TYPE_META.organization.node}
                  fillOpacity={0.1}
                  stroke={BLOCK_TYPE_META.organization.node}
                  strokeOpacity={0.35}
                  strokeWidth={2}
                  filter="url(#hullBlur)"
                />
                ...
                >
                  {BLOCK_TYPE_META.organization.icon} {h.org.name}
                </text>
              </g>
            );
          })}
              ...
              return (
                <g key={r.id} opacity={isDim ? 0.08 : 1} style={{ transition: "opacity .2s" }}>
                  <line
                    ...
                    x2={b.x}
                    y2={b.y}
                    stroke={active ? "rgba(124,58,237,0.55)" : "rgba(130,108,78,0.30)"}
          ...
          {visibleBlocks.map((b) => {
            ...
            return (
              ...
              >
                {(isSelected || isHit) && (
                  <circle r={r + 7} fill="none" stroke="#7c3aed" strokeWidth={2} className="ring-pulse" />
                )}
                {isConflictMode && (
                  <circle r={r + 7} fill="none" stroke="#ef4444" strokeWidth={2} className="ring-pulse" />
                )}
                <circle
                  r={isHover ? r + 3 : r}
                  fill={`url(#grad-${b.type})`}
                  stroke={stroke}
                  strokeWidth={strokeWidth}
                  filter={isSelected || isHit || isConflictMode || scnOrder ? "url(#nodeGlow)" : "url(#softShadow)"}
                ...
                <text
                  ...
                  textAnchor="middle"
                  fontSize="12.5"
                  fontWeight={isSelected || isHover ? 800 : 600}
                  fill={isSelected || isHit ? "#6d28d9" : "#4b463f"}
                  stroke="#faf6ef"
                  strokeWidth={3.5}
                  paintOrder="stroke"
                  style={{ pointerEvents: "none", userSelect: "none" }}
